...
public class Test10 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        totalTimer.Stop();
        UnityEngine.Debug.Log($"[Test10] Generated rooms: {rooms.Count}");
        UnityEngine.Debug.Log($"[Test10] Total generation time: {totalTimer.ElapsedMilliseconds}ms");
    }
    ...
    void SpawnRooms()
    {
        ...
        {
            UnityEngine.Debug.LogWarning("[Test10] No room prefabs assigned.");
            return;
        }
    ...
    void SpawnRoom(Room room, Transform parent)
    {
        ...
        GameObject prefab = roomPrefabs[UnityEngine.Random.Range(0, roomPrefabs.Length)];
    ...
    void ScaleRoom(Transform roomTransform, Room room)
    {
        Vector3 scale = new Vector3(
            room.bounds.width * 1.7f / 10 ,
            1f,
            room.bounds.height * 1.7f / 10
        );
